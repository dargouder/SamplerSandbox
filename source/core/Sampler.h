...
namespace Solstice
{
    ...
    class CameraSample
    {
      public:
        ...
        void BuildPixelSample(double nx_divisor, double ny_divisor, int pixel_x,
                              int pixel_y)
        {
    ...
    void Shuffle(T* p_sample, int p_count, int p_n_dimensions, RNG& p_rng)
    {
        for (int i = 0; i < p_count; ++i)
        {
            ...
            {
                std::swap(p_sample[p_n_dimensions * i + j],
                          p_sample[p_n_dimensions * other + j]);
            }
        }
    }
    ...
    class Sampler
    {
      public:
        /* Sampler Interface */
        Sampler(int64_t p_samples_per_pixel)
            : m_samples_per_pixel(p_samples_per_pixel)
        {
        }
