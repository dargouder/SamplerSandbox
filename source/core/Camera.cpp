...
// Camera::Camera(Vector3 p_position, Vector3 p_direction, float p_fov, float
// p_aspect) :
//        position(p_position), direction(p_direction), fov(p_fov),
//        aspect(p_aspect) {
//
//    up = Vector3(0, 1, 0);
//
...
//
//}
Camera::Camera(Vector3 look_from, Vector3 look_at, Vector3 v_up,
               ...
               float focus_dist)
{
    ...
    _lower_left_corner = _position - _uvw.U * half_width * focus_dist -
                         _uvw.V * half_height * focus_dist -
                         _uvw.W * focus_dist;
...
Ray Camera::generate_ray(const CameraSample& p_camera_sample)
{
    ...
    Vector3 target = _lower_left_corner + _horizontal * s + _vertical * t -
                     _position - offset;
