...
#error Cannot find a function for uint64_t
    ...
    inline bitcount_t trailingzeros(uint32_t v)
    {
        ...
        return multiplyDeBruijnBitPos[((uint32_t)((v & -v) * 0x077CB531U)) >>
                                      27];
    }
    ...
    class uint_x4
    {
      ...
      public:
        ...
        explicit constexpr operator uint64_t() const { return d.v01; }

        explicit constexpr operator uint32_t() const { return w.v0; }

        explicit constexpr operator int() const { return w.v0; }

        explicit constexpr operator uint16_t() const { return w.v0; }

        explicit constexpr operator uint8_t() const { return w.v0; }

        typedef typename std::conditional<
            std::is_same<uint64_t, unsigned long>::value, unsigned long long,
            unsigned long>::type uint_missing_t;

        explicit constexpr operator uint_missing_t() const { return d.v01; }
        ...
        template <typename U, typename V>
        friend std::pair<uint_x4<U, V>, uint_x4<U, V>> divmod(
            const uint_x4<U, V>&, const uint_x4<U, V>&);
    ...
    template <typename UInt, typename UIntX2>
    std::pair<uint_x4<UInt, UIntX2>, uint_x4<UInt, UIntX2>> divmod(
        const uint_x4<UInt, UIntX2>& orig_dividend,
        const uint_x4<UInt, UIntX2>& divisor)
    {
        ...
        // problematic because we can't take the log of zero.  (The boundary
        // case of division by zero is undefined.)
        ...
        if (logdiff == 0) return {ONE, dividend - divisor};
                                     ...
                                     const bitcount_t shift)
    {
        ...
        if (shiftmod)
        {
            ...
            {
#else
            for (uint8_t out = 4 - shiftdiv, in = 4; out != 0;
                 /* dec in loop */)
            {
                --out, --in;
#endif
