...
namespace pcg_extras
{
    ...
    std::basic_ostream<CharT, Traits>& operator<<(
        std::basic_ostream<CharT, Traits>& out, pcg128_t value)
    {
        ...
        if (want_hex)
        {
            ...
            auto desired_width = out._width();
            if (desired_width > 16)
            {
                out._width(desired_width - 16);
            }
    ...
    std::basic_istream<CharT, Traits>& operator>>(
        std::basic_istream<CharT, Traits>& in, pcg128_t& value)
    {
        ...
        if (did_nothing || overflow)
        {
            ...
            if (overflow) current = ~pcg128_t(0ULL);
        }
    ...
    template <typename CharT, typename Traits>
    std::basic_ostream<CharT, Traits>& operator<<(
        std::basic_ostream<CharT, Traits>& out, uint8_t value)
    {
 ...
 */
                               ...
                               DestIter dest_last)
    {
        ...
        return uneven_copy_impl(
            src_first, dest_first, dest_last,
    ...
    template <typename UInt, size_t i = 0UL, size_t N = i + 1UL,
              typename SeedSeq>
     ...
     * initialize a random-number generator some other way, such as from a
     * random device.
     *
     ...
     * that actual engines only called the generate method, so it ought not to
    ...
    class seed_seq_from
    {
      ...
      public:
        ...
        {
            for (auto i = start; i != finish; ++i) *i = result_type(rng_());
        }

        constexpr size_t size() const
        {
            return (sizeof(typename RngType::result_type) >
                        sizeof(result_type) &&
    ...
    // name of a (in human-readable form).  This code allows the idiom:
    //
