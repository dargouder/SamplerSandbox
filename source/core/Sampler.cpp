 ...
 namespace Solstice {
	 ...
	 void LatinHypercube(float* p_samples, int p_n_samples, int p_n_dim, RNG& p_rng)
	 {
		 ...
		 for (int i = 0; i < p_n_samples; ++i)
		 {
			 for (int j = 0; j < p_n_dim; ++j)
			 {
				 ...
				 p_samples[p_n_dim * i + j] = std::min(sj, Maths::OneMinusEpsilon);
			 }
		 }
		 ...
		 for (int i = 0; i < p_n_dim; ++i)
		 {
			 for (int j = 0; j < p_n_samples; ++j)
			 {
				 ...
				 std::swap(p_samples[p_n_dim*j + i], p_samples[p_n_dim*other + i]);
			 }
		 }
	 }
 	...
 	void Sampler::Request2DArray(int n)
 	{
 		...
 		_sample_array_2D.push_back(std::vector<Point2f>(n*m_samples_per_pixel));
 	}
 	...
 	const float * Sampler::Get1DArray(int n)
 	{
 		...
 		return &_sample_array_1D[_array_1D_offset++][_current_pixel_sample_index*n];
 	}
 	...
 	const Point2f * Sampler::Get2DArray(int n)
 	{
 		...
 		return &_sample_array_2D[_array_1D_offset++][_current_pixel_sample_index*n];
 	}
