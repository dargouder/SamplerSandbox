...
namespace Solstice
{
    class Ray
    {
      public:
        Vector3 origin;
        Vector3 direction;
        /* Mutable so they can be modified when intersection rays are const but
         * need to update the hit point using the tmax. */
        mutable float tmin;
        mutable float tmax;
        int depth;
      ...
      public:
        ...
        Ray(const Vector3& p_origin, const Vector3& p_direction,
            const Ray& parent, const float p_min,
            const float p_max = Maths::Infinity)
            : origin(p_origin),
        ...
        Ray(const Ray& p_ray) { *this = p_ray; }
